...
urlpatterns = [
    ...
    # url(r'^viewuser/(?P<username>[\w\-]+)/$', views.view_user, name='view_user'), # this is for all people to be able to view the user information of someone
    url(r'^profile/(?P<username>[\w\-]+)/$', views.profile, name = 'profile'), # this is required by the django-registration-redux
    url(r'^userProfile/(?P<user_name_slug>[\w\-]+)/$', views.userProfile, name = 'userProfile'), #this one is for editing your profile
